...
tab_df2 <- tab_df %>% 
    ...
    bind_cols(issue_no[2], ahead_no[2]) %>%
    ...
    select(Journal, 
           ...
           `Issues per year (n)`,
           `Articles per issue (n; median)`,
           `'ahead of print' articles (n; 31 Dec 2016)`)
...
Other than the European Journal of Pain, which publishes 10 issues per year, the other three journals publish 12 issues per year (Table 2). Yet despite having the lowest issue frequency of the four journals, the European Journal of Pain publishes the second greatest number of articles per issue (median = 14 articles) and hence it is competitive with regards to total number of printed articles per year (median = 140 articles). At the end of 2016 (31 December) the European Journal of Pain had the lowest number of papers of all four journals in _'epub ahead of print'_ format (n = 19 articles). The low number of _'epub ahead of print'_ articles is at odds with the slow _time to print_ data, and the slower issue rate of the journal does not explain the delay in _'time to print'_[^1]. That is, if you miss being published in an issue of the European Journal of Pain, there is a longer wait until the next issue compared to the other three journals, but only by a few days.

[^1]: The final 2016 issue of European Journal of Pain was published in November 2016. The other three journals published an issue in December 2016.

The long _time to print_ for Clinical Journal of Pain isn't easy to explain either. Comparing the data the Clinical Journal of Pain and the Journal of Pain (Table 2), the two journals have comparable issue frequency (12 per year), median articles per issue, and hence total number of articles published per year. Yet, the Clinical Journal of Pain has over three times to number of _'epub ahead of print'_ articles than the Journal of Pain. What ever the reason for the slow _time to print_ speed of the Clinical Journal of Pain, they need to find a solution; increasing the number of articles per issue is the obvious solution. 

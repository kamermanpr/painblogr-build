---
post_title: 'HIV neuropathy: down, but not out'
post_author: 'Peter Kamerman'
post_date: '22 February 2016'
post_image: './images/posts/2016-02-22-hiv-neuropathy-down-but-not-out/post-image.png'
permalink:  '2016-02-22-hiv-neuropathy-down-but-not-out.html'
description: 'New, less toxic HIV treatments mean HIV-sensory neuropathy and its painful consequences are decreasing in prevalence, but they are by no means gone.'
output:
    html_document:
        code_folding: hide
...
![](./images/posts/2016-02-22-hiv-neuropathy-down-but-not-out/ellis-2010-table1.jpg){ style="display:block;margin-right:auto;margin-left:auto;max-width:100%;" }
...
############################################################
# Put into dataframe
extr <- tibble(Neuropathy = c('Yes', 'Yes', 'No', 'No'),
...
############################################################
# Put into dataframe
sim <- tibble(Group = c('ddX_exposure', 'ddX_naive'),
...
df <- right_join(tibble(N = 1:length(ddX_naive),
...
Max <- tibble(Max = c(max(ddX_exposed_3), max(ddX_naive_3)))
Min <- tibble(Min = c(min(ddX_exposed_3), min(ddX_naive_3)))
tooltip_prev <- tibble(tooltip_prev = c(paste0('Point prevalence: ', 
                                               df2$`Prevalence (%)`[1], '%'),
                                        paste0('Point prevalence: ', 
                                               df2$`Prevalence (%)`[2], '%')))
tooltip_95CI <- tibble(tooltip_95CI = c(paste0('Bootstrap 95% CI: ', df2$`Lower 95% CI`[1], 
...
df_chi <- right_join(tibble(N = 1:length(ddX_naive),
                            ddX_naive = factor(ddX_naive)),
                     tibble(N = 1:length(ddX_exposed),
...
fish.1 <- fisher.test(x = ddX_exposure,
                      y = sn_status,
                      simulate.p.value = TRUE, 
                      B = 2000)

fish.2 <- tibble(Estimate = fish.1$estimate, 
                 `95% CI` = paste0(round(fish.1$conf.int[1], 3), ' to ',
...
![](./images/posts/2016-02-22-hiv-neuropathy-down-but-not-out/ellis-2010-table2.jpg){ style="display:block;margin-right:auto;margin-left:auto;max-width:100%;" }
...
############################################################
# Put into dataframe
extr_pain <- tibble(Pain = c('Yes', 'Yes', 'No', 'No'),
                    ddX_exposure = c('Yes', 'No', 'Yes', 'No'),
...
############################################################
# Step 1: Prepare dataframe of simulated datasets (bootstrap sample)
df_pain <- right_join(tibble(N = 1:length(ddX_naive_pain),
...
Max <- tibble(Max = c(max(ddX_exposed_pain_3), max(ddX_naive_pain_3)))
Min <- tibble(Min = c(min(ddX_exposed_pain_3), min(ddX_naive_pain_3)))
tooltip_prev <- tibble(tooltip_prev = c(paste0('Point prevalence: ',
                                               df2_pain$`Prevalence (%)`[1], '%'),
                                        paste0('Point prevalence: ', 
                                               df2_pain$`Prevalence (%)`[2], '%')))
tooltip_95CI <- tibble(tooltip_95CI = c(paste0('Bootstrap 95% CI: ',
...
############################################################
# Organise data for chi-square test
df_pain_chi <- right_join(tibble(N = 1:length(ddX_naive_pain),
                                 ddX_naive = factor(ddX_naive_pain)),
                          tibble(N = 1:length(ddX_exposed_pain),
